...
if($id<>"0"){
 $cuentas=$db->consulta("Select distinct k.serie,k.numero,k.fecha,k.referencia,c.nombre from kardex k, clientes c where k.referencia='$id' and k.tipo='A' and c.id=k.referencia order by k.id desc");
 if($db->numero_de_registros($cuentas)>0){
   ...
   echo "<tbody>";
    while($tr=$db->buscar_array($cuentas)){
      ...
      $busca_cancelacion=$db->consulta("Select * from kardex where serie=$tr[serie] and numero=$tr[numero] and tipo='C'");
      ...
      /*busca todo y suma los articulos del ticket*/
       $busca_compra=$db->consulta("Select sum(cantidad*preciou) as total_ticket from kardex where serie=$tr[serie] and numero=$tr[numero] and tipo='B'");
       ...
       $busca_abonos=$db->consulta("Select sum(preciou) as abonado from kardex where serie=$tr[serie] and numero=$tr[numero] and tipo='A'");
